...
3. Stitch existing `workspace/*.md` files into `WORKSPACE.md` (order: intent → context → research_plan → research_* → synthesis → recommendations → tradeoffs → assumptions → panel_* → next_steps) — this is the only place `WORKSPACE.md` gets regenerated; it is a human-reading convenience, no agent reads it.
